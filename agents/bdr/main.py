...
from agents.bdr.agent import BDRAgent, BDRAgentState
...
app = FastAPI(title="BDR Agent API", version="1.0")
agent = BDRAgent()

class BDRInput(BaseModel):
    lead_id: Optional[str] = None
    context: Dict[str, Any] = {}

@app.get("/health")
async def health_check():
    return {"status": "ok", "agent": "BDR"}

@app.post("/run", response_model=Dict[str, Any])
async def run_agent(input_data: BDRInput, x_service_token: str | None = Header(default=None)):
    try:
        validate_internal_service_token(x_service_token)
        initial_state: BDRAgentState = {
