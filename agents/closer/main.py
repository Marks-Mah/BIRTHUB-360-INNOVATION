...
from agents.closer.agent import CloserAgent, CloserAgentState
...
app = FastAPI(title="Closer Agent API", version="1.0")
agent = CloserAgent()
...
async def health_check():
    return {"status": "ok", "agent": "Closer"}

@app.post("/run", response_model=Dict[str, Any])
async def run_agent(input_data: CloserInput, x_service_token: str | None = Header(default=None)):
    try:
        validate_internal_service_token(x_service_token)
        initial_state: CloserAgentState = {
