...
from agents.sales_ops.agent import SalesOpsAgent, SalesOpsAgentState
...
app = FastAPI(title="Sales Ops Agent API", version="1.0")
agent = SalesOpsAgent()

class SalesOpsInput(BaseModel):
    lead_id: Optional[str] = None
    context: Dict[str, Any] = {}

@app.get("/health")
async def health_check():
    return {"status": "ok", "agent": "SalesOps"}

@app.post("/run", response_model=Dict[str, Any])
async def run_agent(input_data: SalesOpsInput, x_service_token: str | None = Header(default=None)):
    try:
        validate_internal_service_token(x_service_token)
        initial_state: SalesOpsAgentState = {
