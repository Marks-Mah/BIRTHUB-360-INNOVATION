...
import { loadManifestCatalog, type ManifestCatalogEntry } from "@birthub/agents-core";
import { z } from "zod";
...
export async function executeManifestAgentRuntime(
  ...
  const executor = new PlanExecutor({
    hooks: {
      ...
      onStepCompleted: async (step, context) => {
        logs.push(
          `Step ${context.index}/${context.total} concluido com ${step.call.tool}.`
        );
        await persistLogs();
      }
    },
    redis: input.redis,
    tools: createRuntimeTools(resolved.manifest, policyEngine)
  });
  ...
  const toolCost = Math.round(execution.steps.length * 37) / 100;
  const metadata = {
