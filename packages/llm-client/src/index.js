import { OpenAIClient, AnthropicClient, GeminiClient, } from "@birthub/integrations";
...
export class LLMClient {
    ...
    constructor(config) {
        ...
        if (config.providers.openai?.apiKey) {
            this.clients.openai = new OpenAIClient(config.providers.openai.apiKey, config.providers.openai.model);
        }
        if (config.providers.anthropic?.apiKey) {
            this.clients.anthropic = new AnthropicClient(config.providers.anthropic.apiKey, config.providers.anthropic.model);
        }
        if (config.providers.gemini?.apiKey) {
            this.clients.gemini = new GeminiClient(config.providers.gemini.apiKey, config.providers.gemini.model);
        }
    }
